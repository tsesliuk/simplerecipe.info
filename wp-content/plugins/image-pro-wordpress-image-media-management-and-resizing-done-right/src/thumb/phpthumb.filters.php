...
class imagepro_phpthumb_filters {
	...
	function imagepro_phpthumb_filters() {
		return true;
	}
				...
				if ($gdimg_mask_blendtemp = imagepro_phpthumb_functions::ImageCreateFunction(ImageSX($gdimg_image), ImageSY($gdimg_image))) {
					...
					for ($x = 0; $x < ImageSX($gdimg_image); $x++) {
						for ($y = 0; $y < ImageSY($gdimg_image); $y++) {
							...
							$MaskPixel = imagepro_phpthumb_functions::GrayscalePixel(phpthumb_functions::GetPixelColor($gdimg_mask_resized, $x, $y));
	...
	function Desaturate(&$gdimg, $amount, $color='') {
		...
		return imagepro_phpthumb_filters::Colorize($gdimg, $amount, (phpthumb_functions::IsHexColor($color) ? $color : 'gray'));
	}
	...
	function Elipse($gdimg) {
		...
		if ($gdimg_elipsemask_double = imagepro_phpthumb_functions::ImageCreateFunction(ImageSX($gdimg) * 2, ImageSY($gdimg) * 2)) {
			if ($gdimg_elipsemask = imagepro_phpthumb_functions::ImageCreateFunction(ImageSX($gdimg), ImageSY($gdimg))) {
				...
				imagepro_phpthumb_filters::ApplyMask($gdimg_elipsemask, $gdimg);
		...
		if ($gdHist = ImageCreateTrueColor($histW, $histH)) {
			...
			if ($gdHistTemp = ImageCreateTrueColor($HistogramTempWidth, $HistogramTempHeight)) {
				...
				foreach ($BandsToGraph as $key => $band) {
					...
					$thisColor = imagepro_phpthumb_functions::ImageHexColorAllocate($gdHistTemp, phpthumb_functions::IsHexColor(@$Colors[$key]) ? $Colors[$key] : $DefaultColors[$band]);
...
//exit;
		if ($gd_border_canvas = imagepro_phpthumb_functions::ImageCreateFunction($output_width, $output_height)) {
			...
			$color_border = imagepro_phpthumb_functions::ImageHexColorAllocate($gd_border_canvas, (phpthumb_functions::IsHexColor($hexcolor_border) ? $hexcolor_border : '000000'));
