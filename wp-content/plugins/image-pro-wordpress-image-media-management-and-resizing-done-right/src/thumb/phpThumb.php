...
/* if there are any registered handlers opened by other plugins, clean them! */
$outputBuffersCount = count(ob_list_handlers());
for ($obci = 0; $obci < $outputBuffersCount; $obci++) {
    @ob_end_clean();
}
...
function SendSaveAsFileHeaderIfNeeded() {
	...
	global $phpThumb;
	$downloadfilename = imagepro_phpthumb_functions::SanitizeFilename(@$_GET['sia'] ? $_GET['sia'] : (@$_GET['down'] ? $_GET['down'] : 'phpThumb_generated_thumbnail'.(@$_GET['f'] ? $_GET['f'] : 'jpg')));
	if (@$downloadfilename) {
