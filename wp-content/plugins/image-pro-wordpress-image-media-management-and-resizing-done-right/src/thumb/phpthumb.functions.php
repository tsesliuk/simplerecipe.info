...
class imagepro_phpthumb_functions {
	...
	function ImageCreateFunction($x_size, $y_size) {
		...
		if (!function_exists($ImageCreateFunction)) {
			return imagepro_phpthumb::ErrorImage($ImageCreateFunction.'() does not exist - no GD support?');
		}
		if (($x_size <= 0) || ($y_size <= 0)) {
			return imagepro_phpthumb::ErrorImage('Invalid image dimensions: '.$ImageCreateFunction.'('.$x_size.', '.$y_size.')');
		}
		...
		if (empty($gd_info)) {
			...
			if (empty($gd_info['GD Version'])) {
				...
				if (function_exists('ImageCreateFromGIF')) {
					if ($tempfilename = imagepro_phpthumb::phpThumb_tempnam()) {
