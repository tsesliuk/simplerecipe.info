...
//		}
	...
	function resetObject() {
		...
		$this->imagepro_phpthumb(); // re-initialize some class variables
		return true;
	}
			...
			if ($IMtempfilename = $this->phpThumb_tempnam()) {
				...
				if ($this->ra) {
					...
					if ($this->ImageMagickSwitchAvailable('rotate')) {
						if (!eregi('('.implode('|', $this->AlphaCapableFormats).')', $outputFormat) || imagepro_phpthumb_functions::version_compare_replacement($this->ImageMagickVersion(), '6.3.7', '>=')) {
	...
	function Rotate() {
		if ($this->ra || $this->ar) {
			...
			imagepro_phpthumb_filters::ImprovedImageRotate($this->gdimg_source, $rotate_angle, $this->config_background_hexcolor, $this->bg);
			$this->source_width  = ImageSX($this->gdimg_source);
			$this->source_height = ImageSY($this->gdimg_source);
		}
		return true;
			...
			foreach ($this->fltr as $filtercommand) {
				...
				switch ($command) {
					...
					case 'elip': // Elipse cropping
						if (imagepro_phpthumb_functions::gd_version() < 2) {
							$this->DebugMessage('Skipping Elipse() because gd_version is "'.imagepro_phpthumb_functions::gd_version().'"', __FILE__, __LINE__);
							return false;
						}
						$this->is_alpha = true;
					...
					case 'usm': // UnSharpMask sharpening
						...
						if (imagepro_phpthumb_functions::gd_version() >= 2.0) {
							...
							ob_end_clean();
							imagepro_phpUnsharpMask::applyUnsharpMask($this->gdimg_output, $amount, $radius, $threshold);
						} else {
...
//				return $this->ErrorImage('Unknown image type identified by "'.substr($this->rawImageData, 0, 4).'" ('.phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
	...
	function ErrorImage($text, $width=0, $height=0, $forcedisplay=false) {
		impro_log::LogError("[phpThumb] - " . $text);
		/* in case of an error, empty image */
		$pt = new imagepro_phpthumb();
		$pt->setSourceFilename('../view/img/notfound.gif');
		$pt->w = $this->w;
		$pt->h = $this->h;
		$pt->q = 98;
		...
		$this->setOutputFormat();
		if (!$this->thumbnailFormat || (imagepro_phpthumb_functions::gd_version() < 1)) {
